...
function RainPredictionPage() {
  ...
  const handlePredictRain = async () => {
    ...
    try {
      ...
      const mockPRain = Math.random() * 0.8 + 0.1
      const mockAmountIfRain = Math.random() * 15 + 0.5
      const mockExpected = mockPRain * mockAmountIfRain

      setPrediction({
        p_rain: mockPRain,
        amount_if_rain_mm: mockAmountIfRain,
        expected_mm: mockExpected,
      })
    } catch (error) {

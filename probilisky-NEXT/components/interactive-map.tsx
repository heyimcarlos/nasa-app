...
export function InteractiveMap({ latitude, longitude, onLocationChange, expandedMode = false }: InteractiveMapProps) {
  ...
  const drawMap = () => {
    ...
    for (let x = 0; x < tilesWide; x++) {
      for (let y = 0; y < tilesHigh; y++) {
        ...
        img.onload = () => {
          const drawX = x * tileSize - offsetX + width / 2 - (tilesWide * tileSize) / 2
          const drawY = y * tileSize - offsetY + height / 2 - (tilesHigh * tileSize) / 2
  ...
  const drawMarker = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
  ...
  const handleCanvasClick = (e: React.MouseEvent) => {
    ...
    const clickedLat = (180 / Math.PI) * Math.atan(0.5 * (Math.exp(n) - Math.exp(-n)))
  ...
  return (
    <div ref={containerRef} className={`w-full ${expandedMode ? "h-full" : ""}`}>
      <canvas
        ref={canvasRef}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <Button variant="outline" size="sm" onClick={() => setZoom(Math.min(10, zoom + 1))}>
        ...
        <span className="ml-auto text-xs text-muted-foreground">Zoom: {zoom}</span>
      </div>
    </div>
  )
}

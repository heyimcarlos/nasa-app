...
def ensure_unique_sorted_grid(ds: xr.Dataset, lat="lat", lon="lon") -> xr.Dataset:
    ...
    """
    ds = ds.assign_coords(
        {lat: np.round(ds[lat].values.astype(float), 6), lon: np.round(ds[lon].values.astype(float), 6)}
    )
...
def guess_lat_lon_coords(ds: xr.Dataset) -> Tuple[str, str]:
    ...
    """
    lat = next((c for c in ("lat", "latitude", "LAT", "Latitude") if c in ds.coords), None)
    lon = next((c for c in ("lon", "longitude", "LON", "Longitude") if c in ds.coords), None)
...
def get_climatology_dataset(
    ...
    for par in params:
        ...
        clim = da.groupby("time.dayofyear").mean("time", keep_attrs=True).rename(dayofyear="doy")
...
def make_training_table(ds: xr.Dataset, ds_climo: xr.Dataset, target_var: str = TARGET_PARAM) -> pd.DataFrame:
